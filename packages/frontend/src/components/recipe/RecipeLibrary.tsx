...
import { sendRecipeDelete, sendActiveRecipe } from '../../services/socketService';
...
export function RecipeLibrary({ onEdit }: RecipeLibraryProps) {
    // DÜZELTME: setUiMode eylemi store'dan alındı.
    const { savedRecipes, activeRecipe, setActiveRecipe, setUiMode } = useControllerStore();
    const [selectedRecipeId, setSelectedRecipeId] = useState<string | null>(null);

    const handleActivateRecipe = (recipe: Recipe) => {
        setActiveRecipe(recipe);
        sendActiveRecipe(recipe);
        setUiMode('recipe'); // DÜZELTME: UI modunu 'recipe' olarak ayarla.
